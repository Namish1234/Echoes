...
import YouTubeImporter from '@/components/creator/YouTubeImporter';
...
export default function NewEpisodePage() {
  ...
  const [importedAccent, setImportedAccent] = useState('');
  ...
  // Called when YouTube importer confirms generation
  const handleYouTubeImport = (data: {
    title: string;
    guest: string;
    description: string;
    tags: string[];
    ...
    setImportedAccent(data.accentColorHex);
    // Auto-advance to editor after a short delay to let state settle
  ...
  const handleSave = async (status: 'draft' | 'published') => {
    ...
    try {
      await createEpisode({
        ...
        status,
        creatorUid: user.uid,
        ...(spotifyUrl && { spotifyUrl: spotifyUrl.trim() }),
  ...
  if (!podcast) {
    return (
      <div className="p-10">
        <span className="font-bold animate-pulse" style={{ color: 'var(--text-primary)' }}>Loading...</span>
      </div>
    );
  }

  const accent = importedAccent || podcast.accentColor;

  // ── Step 1: Episode info ──
  if (step === 'info') {
    return (
      <div className="p-6 md:p-10 max-w-3xl">
        <Link href={`/creator/podcast/${podcastId}`} className="text-xs font-bold uppercase tracking-widest mb-6 block opacity-60 hover:opacity-100 transition-all" style={{ color: 'var(--text-primary)' }}>
          ← Back to {podcast.title}
        </Link>

        <div className="inline-block px-3 py-1 text-[10px] font-bold tracking-widest uppercase mb-4" style={{ backgroundColor: accent, color: podcast.textOnAccent === 'white' ? '#fff' : '#000' }}>
          New Episode
        </div>
        <h1 className="text-3xl font-black uppercase tracking-tighter mb-8" style={{ color: 'var(--text-primary)' }}>
          Episode <span style={{ color: accent }}>Details</span>
        </h1>
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="flex-1 border-t-2 border-dashed" style={{ borderColor: 'var(--border-subtle)' }} />
          <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>or fill manually</span>
          <div className="flex-1 border-t-2 border-dashed" style={{ borderColor: 'var(--border-subtle)' }} />
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-black uppercase tracking-widest mb-2" style={{ color: 'var(--text-primary)' }}>
                <span className="text-[#1DB954]">Spotify</span> URL (optional)
              </label>
              <input type="url" value={spotifyUrl} onChange={e => setSpotifyUrl(e.target.value)} placeholder="https://open.spotify.com/..." className="w-full zine-border px-4 py-3 font-bold outline-none focus:shadow-[3px_3px_0px_var(--border-color)]" style={{ backgroundColor: 'var(--surface)', color: 'var(--text-primary)' }} />
            </div>
            <div>
              <label className="block text-xs font-black uppercase tracking-widest mb-2" style={{ color: 'var(--text-primary)' }}>
                <span className="text-[#FF0000]">YouTube</span> URL (optional)
              </label>
              <input type="url" value={youtubeUrl} onChange={e => setYoutubeUrl(e.target.value)} placeholder="https://youtube.com/watch?v=..." className="w-full zine-border px-4 py-3 font-bold outline-none focus:shadow-[3px_3px_0px_var(--border-color)]" style={{ backgroundColor: 'var(--surface)', color: 'var(--text-primary)' }} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button onClick={handleAddTag} className="zine-border px-4 py-2 font-black uppercase text-xs cursor-pointer" style={{ backgroundColor: accent, color: podcast.textOnAccent === 'white' ? '#fff' : '#000' }}>Add</button>
            </div>
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tags.map(t => (
                  <span key={t} className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-bold uppercase tracking-widest zine-border" style={{ backgroundColor: accent, color: podcast.textOnAccent === 'white' ? '#fff' : '#000' }}>
                    {t}
        ...
        <div className="mt-10 pt-6 border-t-2 border-dashed flex justify-end" style={{ borderColor: 'var(--border-subtle)' }}>
          <button
            ...
            style={{ backgroundColor: accent, color: podcast.textOnAccent === 'white' ? '#fff' : '#000' }}
          >
            Build Episode →
          </button>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-black uppercase tracking-tighter" style={{ color: 'var(--text-primary)' }}>
            Editing: <span style={{ color: accent }}>{title}</span>
          </h1>
          {sections.length > 0 && (
