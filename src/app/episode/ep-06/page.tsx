      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm z-50 transform transition-transform duration-300 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] flex flex-col ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
        ...
        <div className="p-6 flex flex-col gap-6 flex-1 overflow-y-auto">
          <div className="p-5 zine-border relative overflow-hidden group transition-colors" style={{ backgroundColor: 'rgba(255,107,0,0.08)', borderColor: 'var(--color-wtf-orange)' }}>
            ...
            <Link href="/episode/ep-06/transcript" className="inline-flex w-full items-center justify-center gap-2 px-5 py-3 font-black uppercase tracking-widest text-sm border-2 border-black shadow-[3px_3px_0px_#000] hover:-translate-y-0.5 hover:shadow-[5px_5px_0px_#000] transition-all" style={{ backgroundColor: 'var(--color-wtf-orange)', color: '#000' }}>Open Transcript</Link>
          </div>
...
export default function WTF15Episode() {
  ...
  const episode = episodes.find(ep => ep.id === 'ep-06');
  ...
  const recommendedEpisodes = [
    { id: 'ep-16', title: 'What character flaws make the best entrepreneurs?', guest: 'Ritesh Agarwal, Ghazal Alagh, Manish Poddar', duration: '2 hr +', tags: ['STARTUPS', 'BUSINESS'] },
    { id: 'ep-07', title: 'WTF is Longevity?', guest: 'Bryan Johnson & Panel', duration: '2 hr 15 min', tags: ['HEALTH', 'WELLNESS'] },
    { id: 'ep-03', title: 'WTF does it take to Build Influence Today?', guest: 'Tanmay Bhat & Panel', duration: '2 hr +', tags: ['CREATORS', 'INFLUENCE'] }
  ];
